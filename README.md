...
Jekyll's platform is particularly his Public Safety & Visibility and Institutional Trust policies. These are written as a direct application of Foucauldian surveillance logic. In Jekyll mode this reads as civic policy. In Hyde mode, Hyde explains what it's actually for. Jekyll believes that if he can see everything, he can control everything, including Hyde. Hyde points out that this has not worked. The site's own structure enacts the panopticon's failure as the watcher (Jekyll) cannot stop the watched (Hyde) from speaking.
